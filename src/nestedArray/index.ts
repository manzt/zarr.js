...
export class NestedArray<T extends TypedArray> {
  ...
  constructor(
    data: Buffer | ArrayBuffer | NestedArrayData | null,
    shape: number | number[],
    dtype: DtypeString
  );
  constructor(
    data: Buffer | ArrayBuffer | NestedArrayData | TypedArray | null,
    shape?: number | number[],
    dtype?: DtypeString
  ) {
    ...
    if (this.shape.length === 0) {
      ...
      const numDataElements =
        (data as ArrayBuffer).byteLength / parseInt(dtype[dtype.length - 1], 10);
